...
CHECKPOINT_MODELS=(
    ...
    # Civit AI Models - Format: civitai://<model_id>/<version_id>
    "civitai://4201/5637"  # DreamShaper v8
    "civitai://0/1346244?type=Model&format=SafeTensor&size=pruned&fp=fp16"  # Model with specific parameters
)
UNET_MODELS=()
VAE_MODELS=()
CLIP_MODELS=()
...
function provisioning_download() {
    ...
    if [[ "$url" =~ ^civitai:// ]]; then
        ...
        local model_info=${url#civitai://}
        local model_id=${model_info%%/*}
        local version_id=${model_info#*/}
        
        if ! provisioning_has_valid_civitai_token; then
            echo "No valid Civit AI token found. Skipping download of $url"
            return 1
        fi

        echo "Downloading from Civit AI: Model ID $model_id, Version ID $version_id"
        # Use the Civit AI API to download the model
