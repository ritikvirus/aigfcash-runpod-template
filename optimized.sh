...
# ---------------------------
# AIGFCash Runpod Bootstrapper
# One-pass, first-time setup for ComfyUI + nodes
# - Pins to a stable ComfyUI release tag (default: v0.3.50)
# - Installs ComfyUI's exact Python requirements (incl. frontend)
# - Installs node requirements if present (no more installing nodes as packages)
# - Pre-installs CUDA-matched torchvision/torchaudio to stop torch downgrades/upgrades
# - Uses opencv-contrib-python-headless (replaces opencv-python-headless)
# - Skips deprecated/archived nodes (e.g., ComfyUI-LCM)
# - Retries model downloads and validates size to avoid corruption
# ---------------------------

# ======== Tunables ========
: "${COMFYUI_REF:=v0.3.50}"              # Stable ComfyUI tag (see: https://github.com/comfyanonymous/ComfyUI/releases)
: "${AUTO_UPDATE:=true}"
: "${WORKSPACE:=/workspace}"
...
# Base Python deps (lean)
BASE_PIP_PACKAGES=(
  pip setuptools wheel
  "huggingface_hub==0.25.2"          # keeps cached_download for some nodes
  ...
  soundfile pydub                      # audio nodes
)

# Nodes to clone (skip archived/obsolete ComfyUI-LCM)
NODES=(
...
pipx() {
  # Always use ComfyUI venv if present
...
# Robust download with 3 retries + size check
fetch() {
  ...
  for i in 1 2 3; do
    ...
    # basic corruption guard: ensure file is >256KB
    if [[ -s "$out" && $(stat -c%s "$out") -ge 262144 ]]; then
      echo OK; return 0
    fi
    warn "download retry $i for $url"
    sleep 2
  done
  return 1
}
...
prepare_env() {
  ...
  # Ensure venv is active (ai-dock images provide helpers)
...
install_python_base() {
  ...
  # Force opencv to contrib build to satisfy face_parsing and others
  if python3 -c 'import cv2, sys; print(cv2.__version__)' >/dev/null 2>&1; then
  ...
  # Pre-pin vision/audio to match container torch (avoid nodes pulling a different torch)
...
PY
  then
    log "Installing torchvision/torchaudio that match existing torch..."
    case "$(python3 -c 'import torch;print(torch.__version__.split("+")[0])')" in
      ...
      *) warn "Unknown torch version; skipping pre-pin of torchvision/torchaudio";;
    esac
  fi

  # Install ComfyUI's exact requirements (includes comfyui-frontend-package pinned for the tag)
...
install_nodes() {
  ...
  for repo in "${NODES[@]}"; do
    ...
    # Only install node requirements if present; do NOT `pip install` the node itself
    if [[ -s "$req" ]]; then
  ...
  # Known frequent misses that many nodes assume
...
write_default_graph() {
  if [[ -n "$DEFAULT_WORKFLOW" ]]; then
    ...
    curl -fsSL "$DEFAULT_WORKFLOW" -o /tmp/_wf.json && echo "export const defaultGraph = $(cat /tmp/_wf.json);" > "$js" || true
  fi
}
...
fetch_models() {
  ...
  local d
  # checkpoints
  for d in "${CHECKPOINT_MODELS[@]}"; do
    fetch "$d" "${COMFY_DIR}/models/checkpoints/$(basename "$d")" || warn "failed: $d"
  done
  # unet/clip/lora/controlnet/vae/esrgan
...
PY
  ...
  # A few popular nodes (best-effort)
  pyx - <<'PY'
